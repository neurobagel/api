...
def query_matching_dataset_sizes(dataset_uuids: list, context: dict) -> dict:
    """
    ...
    matching_dataset_size_results = post_query_to_graph(
        util.create_multidataset_size_query(dataset_uuids, context)
    )
    return {
...
async def query_records(
    ...
    matching_dataset_sizes = query_matching_dataset_sizes(
        dataset_uuids=results_df["dataset_uuid"].unique(), context=context
    )
...
async def get_terms(
    data_element_URI: str, terms_vocab: list | None, context: dict
) -> dict:
    """
    ...
    term_url_results = post_query_to_graph(
        util.create_terms_query(data_element_URI, context)
    )
...
async def get_controlled_term_attributes(context: dict) -> list:
    """
    ...
    """
    attributes_query = f"""
    {util.create_query_context(context)}
    ...
    results_list = [
        util.replace_namespace_uri_with_prefix(
            result["attribute"]["value"], context
        )
