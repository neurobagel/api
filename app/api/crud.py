...
async def get_terms(
    data_element_URI: str, terms_vocab_path: Path | None
) -> dict:
    """
    ...
    if terms_vocab_path is not None:
        terms_vocab = util.load_json(terms_vocab_path)
    else:
        terms_vocab = []
...
async def get_term_labels_for_vocab(
    ...
    VocabLabelsResponse
    """
    terms = util.load_json(terms_vocab_path)
    return VocabLabelsResponse(**terms)
